                ...
                <a-table :columns="columns" :row-key="(record) => record.id" :data-source="ebooks"
                    ...
                    <template #bodyCell="{ column, record }">
                        ...
                        <template v-if="column.key === 'category'">
                            <span>{{ getCategoryName(record.category1Id) }}/{{
        ...
        <a-modal title="样本书籍表单" v-model:visible="modalVisible" :confirm-loading="modalLoading" @ok="handleModalok">
            <a-form :v-model="ebook" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
                ...
                <a-form-item label="分类">
                    <a-cascader v-model:value="categoryNames"
...
<!-- categoryIds:{{ categoryIds === null ? '' : categoryIds }}<br>
categoryNames:{{ categoryNames === null ? '' : categoryNames }}<br>
ebook:{{ ebook === null ? '' : ebook }}<br> -->
</template>
...
const showModal = (action: any, record: any) => {
    ...
    if (action === "edit") {
        ...
        categoryNames.value = [
            ...
            getCategoryName(ebook.value.category2Id),
        ];
        
        isModify = true;
    } else {
        ebook.value = {};
        categoryIds.value = Tool.copy(ebook.value);
        isModify = false;
    }
...
const handleModalok = () => {
    ...
    if (isModify) {
        if (!isNaN(Number(categoryNames.value[0]))) {
            
            ebook.value.category1Id = Number(categoryNames.value[0]);        
            ebook.value.category2Id = Number(categoryNames.value[1]);        
        }
    } else {
        // console.log("--------------------------------ebook:" + ebook.value.category1Id);
        console.log("--------------------------------categoryIds:" + categoryNames.value);
        ebook.value.category1Id = Number(categoryNames.value[0]);
        ebook.value.category2Id = Number(categoryNames.value[1]);
    }
